...
let immResourcesRoot = "../nhip/fsh-generated/resources/"


let arFiles = fs.readdirSync(immResourcesRoot);
var url;
arFiles.forEach(function(name){
    ...
    if (name.endsWith('.json')) {
        ...
        let fullFileName = immResourcesRoot + name;    
        ...
        //fix for my server
        if (json.fhirVersion) {
            json.fhirVersion = "4.0.0"
        }

        let url = serverRoot + json.resourceType + "/" + json.id;
        console.log(url)
        PUTFile(url,json)
    }
})
